...
This repository contains the Tekisasu buildroot to generate toolchains for building the Tekisasu engine in a portable way for Linux. Using these toolchains is the best way to distribute Linux builds of your custom-compiled Tekisasu game.
...
The first part of an SDK filename referes to the architecture that this SDK will generate binaries *for*. If you want to ship your game to both 32bit and 64bit Intel/AMD users you will need both `x86_64-tekisasu-linux-gnu_sdk-buildroot.tar.gz` and `i686-tekisasu-linux-gnu_sdk-buildroot.tar.gz`.
...
*Using this method will create a toolchain you yourself can use to create Tekisasu releases that will work on any Linux system currently in use. However the toolchain you generate will not be portable to older Linuxes. If you plan to distribute the toolchain itself use the podman method below*

The basic steps for building a toolchain are:

 * copy `config-tekisasu-<arch>` to `.config`
 * run `make olddefconfig`
 * run `make clean sdk`

Afterwards the SDK will be in `output/images/<arch>-tekisasu-linux-gnu_sdk-buildroot.tar.gz`.
...
The toolchain will appear in the `tekisasu-toolchains` directory
 ...
 * Copy the architecture you would like to change to `.config` for instance `cp config-tekisasu-x86_64 .config`
 * Run `make menuconfig`
 * Make your changes

At this point your changes exist in .config. **Make a backup**. If you're building for local use just run `make clean sdk`, if you're using the container approach copy your `.config` file to the arch config like `config-tekisasu-x64_64`
